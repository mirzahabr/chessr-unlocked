# Chessr.io v3.4.0 — Reverse-Engineered & Unlocked
...
This version was processed by the `chessr-version-update` skill from the previous
version, `chessr v3.3.2/`. See `project-documentation/PROJECT_REPORT.md` §9.1 inside
either build for exactly what changed between the two versions.
...
the `chessr-unlocked-all-features-*.tar.gz` for this version. You don't need to clone the
repo for that.
...
- **Stream Mode unlocked** — the same changes are mirrored in the separate Stream Mode
  bundle (`chunks/stream-CcAo12Z7.js`).
- **New in this version**: the extension's own "price increase" upsell banner (added
  upstream in v3.4.0) is confirmed gated by the same mechanism as every other upgrade
  prompt, so it's suppressed the same way — see §9.1/§10.8 in the report.
...
- The engine suite (Stockfish, Dragon/Komodo, Rodent IV, Maia 3, Torch) and move
  classification
- **§9** — the deobfuscation methodology, the behavior-preservation gate, and
  **§9.1 What's New vs the previous version**
- **§10** (unlocked build only) — the account system's architecture, exactly what was
  removed/neutralized to unlock it, and the one feature that cannot be unlocked
  client-side
...
For the cross-version knowledge base (accumulated understanding across every version
processed, not just this one), see the repo root's `knowledge-base/` folder, or the
frozen snapshot inside `chessr-unlocked-all-features/project-documentation/knowledge-base/`.
