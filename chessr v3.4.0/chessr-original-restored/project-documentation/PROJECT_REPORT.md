...
- **New in v3.4.0**: a "price increase" upsell feature layered inside the
  existing free-tier upgrade modal (account-gated; not applicable to this
  untouched restoration) — see §9.1.
...
- **New feature — a "price increase" upsell**: a new `FreeUpgradeModal.tsx` /
  `PriceIncreasePlans.tsx` component pair appears in the restored code: a
  countdown banner ("your price stays locked while your subscription is
  active... ends in `{{time}}`") showing monthly/yearly/lifetime/one-time
  pricing tiers with a "best deal" badge, plus a small announcement badge
  next to the plan indicator in Settings. 12 new i18n keys
  (`upgrade.increase.*`) across all 8 shipped languages. This component is
  part of the original account/subscription system and is therefore
  unmodified in this untouched restoration; the sibling
  `chessr-unlocked-all-features/` build's report documents how it is gated
  and why no new modification was needed there.
- **Account/premium architecture**: the account/subscription system's
  overall shape (auth store, plan-gating predicates, linked-accounts store,
  Discord store, ELO/engine-strength limits) is confirmed structurally
  unchanged from v3.3.2 — same field names, same method names, same
  enforcement sites, at identical occurrence counts. This restoration made
  no changes to any of it; see the `chessr-unlocked-all-features/` build's
  report for the full architecture write-up and how it is neutralized there.
