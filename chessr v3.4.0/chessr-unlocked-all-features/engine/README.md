...
deobfuscation, reformatting, renaming, or unlock-related editing is ever performed here,
in any version — this is a hard boundary. Documentation only; see
`project-documentation/PROJECT_REPORT.md` §6 for full detail.
...
| Rodent | `rodent/` | Personality-based engine (39 personalities when unlocked). |
...
Premium gating (ELO caps, personality restrictions, etc.) happens entirely at the UI/store
level in `content-scripts/content.js` and the Stream Mode bundle — never inside these
engine files themselves.
